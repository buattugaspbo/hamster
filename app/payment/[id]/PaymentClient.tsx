...
export function PaymentClient({ id }: { id: string }) {
  ...
  if (order?.paymentStatus === "Dibayar") {
    return <div className="payment-success"><span>✓</span><p className="eyebrow">PEMBAYARAN DITERIMA</p><h1>Berhasil. Pesananmu sudah tercatat.</h1><p>Tim HOP & HAM akan melanjutkan konfirmasi dan persiapan pesanan.</p><div><Link href={`/account/orders/${id}`} className="button button--solid">Lihat pesanan</Link><Link href="/shop" className="button button--outline">Kembali belanja</Link></div></div>;
  }

  return (
    <div className="payment-page content-shell">
      <header className="payment-heading"><div><p className="eyebrow">QRIS DINAMIS</p><h1>Selesaikan pembayaran</h1><p>Pesanan <strong>{id}</strong> · nominal otomatis muncul saat dipindai</p></div><div className="payment-timer"><span>{hours}</span><i>:</i><span>{minutes}</span><i>:</i><span>{seconds}</span><small>JAM&nbsp;&nbsp;&nbsp;&nbsp;MENIT&nbsp;&nbsp;&nbsp;&nbsp;DETIK</small></div></header>
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="payment-grid payment-grid--qris">
        <section className="payment-methods"><h2>Ringkasan pesanan</h2>{order?.items.map((item) => <div className="payment-order-row" key={item.productName}><span>{item.productName} × {item.quantity}</span><strong>{formatRupiah(item.unitPrice * item.quantity)}</strong></div>)}<div className="payment-security"><span>⌾</span><p><strong>Nominal divalidasi server</strong><small>Harga tidak diambil dari browser. QR dibuat khusus untuk total pesanan ini.</small></p></div></section>
        <aside className="payment-instruction"><div className="payment-card-head"><span>Total pembayaran</span><strong>{order ? formatRupiah(order.total) : "Memuat…"}</strong></div><div className="qris-head"><b>QRIS</b><span>NMID: ID1026534598481</span></div>{qr ? <>{/* eslint-disable-next-line @next/next/no-img-element */}<img className="qris-dynamic-image" src={qr} alt={`QRIS pembayaran ${formatRupiah(order?.total || 0)}`} /><p className="payment-note">Pindai dengan aplikasi bank atau e-wallet. Periksa nama merchant dan nominal sebelum membayar.</p><a href={qr} download={`QRIS-${id}.png`} className="button button--outline">Unduh QRIS</a></> : <div className="qr-loading">Membuat QRIS sesuai total…</div>}<button onClick={sendPaymentNotice} disabled={!qr || noticeLoading || order?.paymentStatus === "Menunggu verifikasi"} className="button button--solid payment-confirm">{order?.paymentStatus === "Menunggu verifikasi" ? "Menunggu verifikasi admin" : noticeLoading ? "Mengirim…" : "Saya sudah membayar"}</button><small className="prototype-note">Tombol ini mengirim pemberitahuan. Status “Dibayar” hanya diberikan admin atau webhook acquirer.</small></aside>
      </div>
