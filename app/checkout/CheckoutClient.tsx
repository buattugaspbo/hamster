...
export function CheckoutClient() {
  ...
  return (
    <div className="checkout-page content-shell">
      ...
      <div className="checkout-account-note"><span>○</span><p><strong>Sudah punya akun?</strong> Masuk agar pesanan otomatis tersimpan di akunmu.</p><a href="/login?next=/checkout">Masuk</a></div>
      ...
      <form className="checkout-grid" onSubmit={submit}>
        <section className="checkout-form">
          ...
          {!animalId && <>
            ...
            <div className="shipping-calculator" aria-live="polite"><div><span>Ongkir dari Palembang</span>{shippingLoading ? <strong>Menghitung…</strong> : shippingQuote ? <strong>{formatRupiah(shippingQuote.cost)}</strong> : <strong>Pilih kota tujuan</strong>}</div>{shippingQuote && <p>Biaya dasar Rp10.000 + {shippingQuote.distanceKm.toLocaleString("id-ID")} km × Rp49/km, lalu dibulatkan ke Rp500 terdekat.</p>}<small>Estimasi menggunakan jarak garis lurus antarkoordinat kota.</small></div>
          </>}
        ...
        <aside className="checkout-summary"><h2>Ringkasan</h2>{selected.map(({ product, quantity }) => <div className="summary-item" key={product.id}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={product.image} alt={product.name} /><div><strong>{product.name}</strong><span>{quantity} × {product.breed || product.category}</span></div><b>{formatRupiah(product.price * quantity)}</b></div>)}{!animalId && <div className="summary-shipping"><span>Subtotal</span><strong>{formatRupiah(subtotal)}</strong><span>Ongkir</span><strong>{shippingQuote ? formatRupiah(shippingCost) : "—"}</strong></div>}<div className="summary-total"><span>Total</span><strong>{formatRupiah(grandTotal)}</strong></div><p className="reservation-note">Total final dihitung ulang oleh server berdasarkan harga, stok, dan kota tujuan.</p><button disabled={loading || !selected.length || (!animalId && !shippingQuote)} className="button button--solid" type="submit">{loading ? "Membuat pesanan…" : "Buat pesanan & tampilkan QRIS"}</button></aside>
      </form>
    </div>
  );
}
